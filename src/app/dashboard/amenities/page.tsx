...
export default function AmenitiesDashboard() {
  ...
  useEffect(() => {
    const loadAmenities = async () => {
      try {
        setIsLoading(true);
        const response: any = await fetchAmenities();
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-2 opacity-80">Manage venue amenities information</p>
              </div>
              
              {!isLoading && !error && (
                <div className="px-4 py-2 bg-white/10 rounded-lg">
                  <span className="text-sm font-medium">Total Amenities: <span className="text-white font-bold">{totalItems}</span></span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-xl overflow-hidden border border-gray-100 p-6">
          {error ? (
            <div className="text-center p-8">
              ...
              <h2 className="text-2xl font-bold text-gray-800">Error Loading Amenities</h2>
            ...
            <div className="flex items-center justify-center h-48">
              <div className="text-center">
                ...
                <p className="text-gray-500 mt-4">Loading amenities...</p>
              </div>
            </div>
          ) : (
            <div className="flex items-center justify-center h-48">
              <div className="text-center">
                ...
                <h2 className="text-2xl font-bold text-gray-800">Amenities Dashboard</h2>
                <p className="text-gray-500 mt-2">{totalItems} amenities available in the system</p>
              </div>
            </div>
          )}
        </div>
